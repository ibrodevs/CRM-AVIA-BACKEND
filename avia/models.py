"""Авиа: маршруты, тарифы, брони, билеты (ТЗ §9.1, ограничения §30)."""
...
class FlightItinerary(TenantModel):
    service = models.OneToOneField("services.OrderService", on_delete=models.CASCADE,
                                   related_name="flight_itinerary")
    kind = models.CharField(max_length=12, default="one_way")  # one_way/round_trip/multi_city
...
class FlightSegment(TenantModel):
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=["itinerary", "sequence"],
                                    name="uniq_avia_segment_sequence"),
        ]
        ordering = ["sequence"]


class AviaFare(TenantModel):
    service = models.ForeignKey("services.OrderService", on_delete=models.CASCADE,
                                related_name="avia_fares")
    passenger = models.ForeignKey("services.ServicePassenger", null=True, blank=True,
                                  on_delete=models.CASCADE, related_name="avia_fares")
    cabin = models.CharField(max_length=16)  # economy/premium_economy/business/first
...
class AviaBooking(TenantModel):
    ...
    class Status(models.TextChoices):
        ...
        TICKETED = "ticketed"
        CANCELLED = "cancelled"
        UNKNOWN = "unknown"  # ambiguous timeout — повтор запрещён до retrieve
        FAILED = "failed"

    service = models.ForeignKey("services.OrderService", on_delete=models.PROTECT,
                                related_name="avia_bookings")
    provider_adapter = models.CharField(max_length=100)
    environment = models.CharField(max_length=16, default="sandbox")
    locator = models.CharField(max_length=16, blank=True)  # PNR
    client_request_id = models.CharField(max_length=80)    # идемпотентность брони
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.UniqueConstraint(fields=["tenant", "client_request_id"],
                                    name="uniq_avia_client_request"),
        ]
...
class Ticket(TenantModel):
    ...
    status = models.CharField(max_length=16, default="issued")  # issued/voided/refunded/exchanged
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=["validating_carrier", "ticket_number"],
...
class EMD(TenantModel):
    ...
    passenger = models.ForeignKey("services.ServicePassenger", null=True, blank=True,
                                  on_delete=models.PROTECT, related_name="emds")
    emd_number = models.CharField(max_length=20)
    validating_carrier = models.CharField(max_length=3)
    reason = models.CharField(max_length=100, blank=True)  # baggage/seat/meal/...
    status = models.CharField(max_length=16, default="issued")

    class Meta:
        db_table = "avia_emd"
        constraints = [
            models.UniqueConstraint(fields=["validating_carrier", "emd_number"],
                                    name="uniq_emd_number"),
        ]
...
class SeatAssignment(TenantModel):
    ...
    passenger = models.ForeignKey("services.ServicePassenger", on_delete=models.CASCADE,
                                  related_name="seat_assignments")
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["segment", "seat_number"], condition=models.Q(status="active"),
                name="uniq_active_seat",
            ),
            models.UniqueConstraint(
                fields=["segment", "passenger"], condition=models.Q(status="active"),
...
class AncillaryAssignment(TenantModel):
    ...
    service = models.ForeignKey("services.OrderService", on_delete=models.CASCADE,
                                related_name="avia_ancillaries")
    passenger = models.ForeignKey("services.ServicePassenger", on_delete=models.CASCADE,
                                  related_name="ancillaries")
    segment = models.ForeignKey(FlightSegment, null=True, blank=True,
                                on_delete=models.CASCADE, related_name="ancillaries")
    ...
    price = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True)
    currency = models.CharField(max_length=3, blank=True)
    status = models.CharField(max_length=12, default="selected")  # selected/confirmed/issued/cancelled
