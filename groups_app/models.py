...
class PassengerGroup(TenantModel):
    ...
    owner = models.ForeignKey(
        "accounts.User", null=True, blank=True, on_delete=models.SET_NULL, related_name="owned_groups"
    )
...
class GroupSubgroup(TenantModel):
    group = models.ForeignKey(PassengerGroup, on_delete=models.CASCADE, related_name="subgroups")
...
class GroupOrder(TenantModel):
    ...
    order = models.OneToOneField("orders.Order", on_delete=models.PROTECT, related_name="group_order")
    group = models.ForeignKey(
        PassengerGroup, null=True, blank=True, on_delete=models.PROTECT, related_name="group_orders"
    )
    scenario = models.CharField(max_length=20, choices=Scenario.choices, default=Scenario.CLASSIC_BLOCK)
    airline = models.CharField(max_length=3, blank=True)
    supplier = models.ForeignKey(
        "suppliers.Supplier", null=True, blank=True, on_delete=models.PROTECT, related_name="group_orders"
    )
    ...
    split_state = models.JSONField(null=True, blank=True)
...
GROUP_ORDER_TRANSITIONS: dict[str, set[str]] = {
    ...
    GroupOrder.Status.CONFIRMED: {
        GroupOrder.Status.NAMES_PENDING,
        GroupOrder.Status.REDUCED,
        GroupOrder.Status.CANCELLED,
    },
    GroupOrder.Status.NAMES_PENDING: {
        GroupOrder.Status.NAMES_SUBMITTED,
        GroupOrder.Status.REDUCED,
        GroupOrder.Status.CANCELLED,
    },
    GroupOrder.Status.NAMES_SUBMITTED: {
        GroupOrder.Status.PARTIALLY_TICKETED,
...
class GroupBlock(TenantModel):
    ...
    group_order = models.ForeignKey(GroupOrder, on_delete=models.CASCADE, related_name="blocks")
    name = models.CharField(max_length=150)
...
class GroupPassengerAssignment(TenantModel):
    ...
    block = models.ForeignKey(GroupBlock, on_delete=models.CASCADE, related_name="assignments")
    participant = models.ForeignKey(
        "orders.OrderParticipant", on_delete=models.PROTECT, related_name="group_assignments"
    )
...
class GroupRequest(TenantModel):
    ...
    group_order = models.ForeignKey(GroupOrder, on_delete=models.CASCADE, related_name="requests")
...
class GroupSupplierResponse(TenantModel):
    request = models.ForeignKey(GroupRequest, on_delete=models.CASCADE, related_name="responses")
    body = models.TextField(blank=True)
...
class RosterImportJob(TenantModel):
    ...
    order = models.ForeignKey("orders.Order", on_delete=models.CASCADE, related_name="roster_imports")
...
class RosterMergeHistory(models.Model):
    ...
    import_job = models.ForeignKey(RosterImportJob, on_delete=models.CASCADE, related_name="merge_history")
