...
def group_order_setup(admin_client, tenant, admin_user):
    persons = [
        Person.objects.create(
            tenant=tenant,
            surname=f"Пассажир{i}",
            given_name="Тест",
            latin_surname=f"PASSAZHIR{i}",
            latin_given_name="TEST",
    ...
    order = admin_client.post(
        ...
        {
            "request_type": "group",
            "client_person": str(persons[0].id),
            "participants": [{"person": str(p.id)} for p in persons],
        },
        format="json",
    ).json()
    group_order = admin_client.post(
        "/api/v1/group-orders/",
        {
            "order": order["id"],
            "scenario": "classic_block",
            "requested_seats": 10,
        },
        format="json",
    ).json()
    block = admin_client.post(
        f"/api/v1/group-orders/{group_order['id']}/blocks/",
        {
            "name": "Блок A",
            "seats": 2,
        },
...
class TestGroups:
    def test_mass_assign_partial_success(self, admin_client, group_order_setup):
        ...
        participants = admin_client.get(f"/api/v1/orders/{order['id']}/participants/").json()
        items = [{"participant_id": p["id"], "block_id": block["id"]} for p in participants]
        response = admin_client.post(
            f"/api/v1/group-orders/{group_order['id']}/mass-actions/",
            {"action": "assign_block", "items": items},
            format="json",
        )
        body = response.json()
    ...
    def test_matrix(self, admin_client, group_order_setup):
        ...
        matrix = admin_client.get(f"/api/v1/group-orders/{group_order['id']}/matrix/").json()
...
class TestRosterImport:
    def test_import_preview_apply(self, admin_client, group_order_setup):
        ...
        file = SimpleUploadedFile("roster.csv", CSV_CONTENT.encode("utf-8"), content_type="text/csv")
        response = admin_client.post(
            "/api/v1/roster-imports/", {"order": order["id"], "file": file}, format="multipart"
        )
        ...
        assert states[0] == "same"
        assert states[1] == "new"
        assert preview["stats"]["invalid"] >= 1
        assert preview["stats"]["missing"] == 2
    ...
    def test_export_translit(self, admin_client, group_order_setup):
        order, _, _, _ = group_order_setup
        file = SimpleUploadedFile("roster.csv", CSV_CONTENT.encode("utf-8"), content_type="text/csv")
        import_id = admin_client.post(
            "/api/v1/roster-imports/", {"order": order["id"], "file": file}, format="multipart"
        ).json()["id"]
        response = admin_client.get(f"/api/v1/roster-imports/{import_id}/export/?format=csv")
...
class TestDocuments:
    ...
    def test_mime_spoofing_rejected(self, admin_client):
        ...
        file = SimpleUploadedFile("fake.pdf", b"<html>not a pdf</html>", content_type="application/pdf")
        response = admin_client.post(
            "/api/v1/documents/",
            {
                "file": file,
                "document": json.dumps({"kind": "other", "title": "x"}),
            },
            format="multipart",
        )
    ...
    def test_correction_version(self, admin_client):
        document = self._create(admin_client)
        file = SimpleUploadedFile("fixed.pdf", b"%PDF-1.4 fixed", content_type="application/pdf")

        response = admin_client.post(
            f"/api/v1/documents/{document['id']}/versions/", {"file": file}, format="multipart"
        )
        assert response.status_code == 400
        file = SimpleUploadedFile("fixed.pdf", b"%PDF-1.4 fixed", content_type="application/pdf")
        response = admin_client.post(
            f"/api/v1/documents/{document['id']}/versions/",
            {"file": file, "reason": "опечатка в фамилии"},
            format="multipart",
        )
    ...
    def test_confidential_hidden_without_permission(self, admin_client, operator_user):
        ...
        file = SimpleUploadedFile("passport.pdf", self.PDF, content_type="application/pdf")
        document = admin_client.post(
            "/api/v1/documents/",
            {
                "file": file,
                "document": json.dumps({"kind": "passport", "title": "Паспорт", "is_confidential": True}),
            },
            format="multipart",
        ).json()
...
class TestChat:
    def test_thread_send_read_unread(self, admin_client, operator_user, admin_user):
        ...
        admin_client.post(
            f"/api/v1/chat/threads/{thread['id']}/participants/",
            {"user": str(operator_user.id)},
            format="json",
        )
        message = admin_client.post(
            f"/api/v1/chat/threads/{thread['id']}/send/",
    ...
    def test_messages_cursor(self, admin_client):
        thread = admin_client.post(
            "/api/v1/chat/threads/", {"type": "internal", "title": "т"}, format="json"
        ).json()
        for i in range(5):
            admin_client.post(
                f"/api/v1/chat/threads/{thread['id']}/send/", {"body": f"msg {i}"}, format="json"
            )
        page = admin_client.get(f"/api/v1/chat/threads/{thread['id']}/messages/?limit=3").json()
    ...
    def test_webhook_dedup(self, db, tenant):
        ...
        first = client.post("/api/v1/communications/webhooks/telegram/", payload, format="json")
        assert first.status_code == 200
        second = client.post("/api/v1/communications/webhooks/telegram/", payload, format="json")
