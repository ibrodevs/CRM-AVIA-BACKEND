"""ЖД: поезд, вагон, места, удержание (ТЗ §9.2)."""
...
class RailSegment(TenantModel):
    service = models.ForeignKey("services.OrderService", on_delete=models.CASCADE,
                                related_name="rail_segments")
    sequence = models.PositiveSmallIntegerField(default=1)
    train_number = models.CharField(max_length=16)
    train_type = models.CharField(max_length=32, blank=True)  # скорый/фирменный/...
    ...
    service_class = models.CharField(max_length=16, blank=True)  # купе/плацкарт/СВ/сидячий
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=["service", "sequence"],
                                    name="uniq_rail_segment_sequence"),
        ]
...
class RailSeatHold(TenantModel):
    ...
    passenger = models.ForeignKey("services.ServicePassenger", null=True, blank=True,
                                  on_delete=models.CASCADE, related_name="rail_seats")
    seat_number = models.CharField(max_length=8)
    seat_type = models.CharField(max_length=16, blank=True)  # нижнее/верхнее/у окна
