...
class GlobalSearchView(APIView):
    ...
    def get(self, request):
        ...
        if len(q) < 2:
            raise ApiError(code="QUERY_TOO_SHORT", message="Минимум 2 символа", status_code=400)
        ...
        if "order" in types and has_permission(user, "orders.view"):
            ...
            for order in (
                orders_visible_to(user)
                .filter(Q(number__icontains=q) | Q(purpose__icontains=q))
                .select_related("client_person", "client_company")[:limit]
            ):
                client = (
                    order.client_person.full_name
                    if order.client_person_id
                    else str(order.client_company)
        ...
        if "person" in types and has_permission(user, "crm.view"):
            ...
            for person in Person.objects.filter(tenant_id=user.tenant_id, archived_at__isnull=True).filter(
        ...
        if "company" in types and has_permission(user, "crm.view"):
            ...
            for company in Company.objects.filter(tenant_id=user.tenant_id, archived_at__isnull=True).filter(
                Q(legal_name__icontains=q) | Q(short_name__icontains=q) | Q(tax_id__icontains=q)
            )[:limit]:
                results.append(
                    {
                        "type": "company",
                ...
                results.append(
                    {
                        ...
                        "deep_link": f"/suppliers/{supplier.id}",
                        "score": 0.5,
                    }
                )
