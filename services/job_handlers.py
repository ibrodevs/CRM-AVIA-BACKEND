"""Фоновое выполнение поиска по поставщикам (ТЗ §8.2)."""
import hashlib
import json
...
def _dedup_hash(kind: str, itinerary: dict, price_amount: str, currency: str) -> str:
    return hashlib.sha256(
        json.dumps({"kind": kind, "itinerary": itinerary, "price": price_amount,
                    "currency": currency}, sort_keys=True, default=str).encode()
    ).hexdigest()


def _suppliers_for_search(session: SearchSession) -> list[Supplier | None]:
    priority = SupplierSearchPriority.objects.filter(
        tenant_id=session.tenant_id, service_kind=session.kind,
        is_active=True, archived_at__isnull=True,
    ).first()
    if priority and priority.ordered_suppliers:
        suppliers = list(Supplier.objects.filter(pk__in=priority.ordered_suppliers,
    ...
    suppliers = list(Supplier.objects.filter(
        tenant_id=session.tenant_id, status=Supplier.Status.ACTIVE,
        service_kinds__contains=[session.kind], archived_at__isnull=True,
    ))
    return suppliers or [None]  # None => прямой mock-адаптер без поставщика
...
def run_search(job: BackgroundJob) -> dict:
    ...
    for supplier in suppliers:
        adapter_key = "mock"  # реальный ключ адаптера берётся из credentials поставщика
        if supplier is not None:
            credential = supplier.credentials.filter(archived_at__isnull=True,
                                                     status="active").first()
            if credential is not None:
                adapter_key = credential.provider_adapter
        run = SearchProviderRun.objects.create(
            tenant_id=session.tenant_id, session=session, supplier=supplier,
            provider_adapter=adapter_key, status=SearchProviderRun.Status.RUNNING,
            started_at=timezone.now(),
        )
        ctx = AdapterContext(tenant_id=session.tenant_id,
        ...
        try:
            ...
            # Ошибка одного поставщика не обнуляет остальных (ТЗ §8.2).
            run.status = (SearchProviderRun.Status.TIMEOUT if exc.category == "timeout"
                          else SearchProviderRun.Status.FAILED)
            run.error_code = exc.code
            run.completed_at = timezone.now()
            run.save(update_fields=["status", "error_code", "completed_at"])
            failed += 1
            emit_event("search.progress", session,
                       payload={"supplier": str(supplier.id) if supplier else None,
        ...
        for raw in raw_offers:
            ...
            dedup = _dedup_hash(session.kind, raw.get("itinerary", {}),
                                str(base), currency)
            if dedup in seen_hashes:
                continue  # дедупликация эквивалентных офферов
            seen_hashes.add(dedup)

            markup_rules = resolve_markup_rules(
                supplier, kind=session.kind,
                route=_criteria_route(session.criteria),
                cabin=str(session.criteria.get("cabin", "")),
            )
            offer = ServiceOffer.objects.create(
                tenant_id=session.tenant_id, session=session, kind=session.kind,
                supplier=supplier, provider_adapter=adapter_key,
                external_key=raw.get("external_key", ""),
                itinerary=raw.get("itinerary", {}),
                fare=raw.get("fare"),
                price_amount=base, price_currency=currency,
                availability=raw.get("availability", ""),
                expires_at=raw.get("expires_at"),
                raw_snapshot=raw, dedup_hash=dedup,
            )
            pricing = calculate_price(
                base=base, currency=currency, markup_rules=markup_rules,
                tenant_id=session.tenant_id, offer=offer, step="search",
            )
            offer.price_amount = pricing["total"]
            offer.applied_markup_rules = [c for c in pricing["components"]
                                          if c["name"] == "supplier_markup"]
        ...
        emit_event("search.progress", session,
                   payload={"supplier": str(supplier.id) if supplier else None,
                            "status": "succeeded", "offers": created},
                   audience_user=session.user)
    ...
    emit_event("search.completed" if succeeded else "search.failed", session,
