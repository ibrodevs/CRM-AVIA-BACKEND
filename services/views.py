"""Services API: поиск, офферы, услуги заказа (ТЗ §8)."""
...
from services.models import (
    SERVICE_TRANSITIONS, OrderService, SearchProviderRun, SearchSession, ServiceExtra,
    ServiceOffer,
)
...
class SearchSessionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = SearchSession
        fields = ["id", "kind", "criteria", "status", "order", "expires_at", "created_at",
                  "provider_runs"]

    def get_provider_runs(self, obj):
        return [
            {"id": str(r.id), "supplier": str(r.supplier_id) if r.supplier_id else None,
             "provider_adapter": r.provider_adapter, "status": r.status,
             "offers_count": r.offers_count, "error_code": r.error_code}
...
class ServiceOfferSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ServiceOffer
        fields = ["id", "kind", "supplier", "provider_adapter", "external_key", "is_manual",
                  "itinerary", "fare", "price", "availability", "expires_at",
                  "applied_markup_rules", "compliance", "created_at"]
...
class OrderServiceSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = OrderService
        fields = ["id", "order", "kind", "status", "title", "supplier", "external_id",
                  "source", "starts_at", "ends_at", "currency", "supplier_cost", "taxes",
                  "agency_fee", "markup", "commission", "discount", "client_total",
                  "client_price", "payment_deadline", "ticketing_deadline", "responsible",
                  "policy_compliance", "cancellation_rules", "version", "created_at"]
...
class ServiceExtraSerializer(serializers.ModelSerializer):
    class Meta:
        model = ServiceExtra
        fields = ["id", "catalog_item", "name", "stage", "availability", "passenger",
                  "quantity", "price", "currency", "fee", "status", "emd_reference"]
...
class SearchCreateView(APIView):
    ...
    def post(self, request):
        ...
        if kind not in [k for k, _ in SearchSession._meta.get_field("kind").choices]:
            raise ApiError(code="VALIDATION_ERROR", message="Неизвестный kind",
                           fields={"kind": ["Недопустимое значение"]}, status_code=400)
        if not isinstance(criteria, dict):
            raise ApiError(code="VALIDATION_ERROR", message="criteria обязателен",
                           fields={"criteria": ["Обязательный объект"]}, status_code=400)
        if not has_service_action(request.user, kind, "search"):
            raise ApiError(code="SERVICE_KIND_FORBIDDEN",
                           message=f"Нет доступа к поиску {kind}", status_code=403)
        order = None
        if order_id := request.data.get("order"):
            order = get_order_or_404(request.user, order_id)
        session = SearchSession.objects.create(
            tenant_id=request.user.tenant_id, user=request.user, order=order,
            kind=kind, criteria=criteria, created_by=request.user,
        )
        job = enqueue("services.search", {"session_id": str(session.id)}, request=request)
        session.job = job
        session.save(update_fields=["job"])
        return Response({"search_id": str(session.id), "job_id": str(job.id)},
                        status=http.HTTP_202_ACCEPTED)


def _get_session(request, search_id) -> SearchSession:
    session = SearchSession.objects.filter(pk=search_id,
                                           tenant_id=request.user.tenant_id).first()
    if session is None:
        raise ApiError(code="NOT_FOUND", message="Поисковая сессия не найдена",
                       status_code=404)
    return session
...
class SearchOffersView(GenericAPIView):
    ...
    def get(self, request, search_id):
        ...
        if sort == "cheap":
            ...
            qs = qs.order_by("price_amount")  # duration в itinerary; сортировка уточняется
        elif sort == "early":
...
class OfferRevalidateView(APIView):
    ...
    def post(self, request, offer_id):
        ...
        ctx = AdapterContext(tenant_id=request.user.tenant_id,
        ...
        return Response({"offer": ServiceOfferSerializer(offer).data,
                         "revalidation": result})
...
class OfferCompareView(APIView):
    ...
    def post(self, request):
        ...
        if not isinstance(ids, list) or not 2 <= len(ids) <= 5:
            raise ApiError(code="VALIDATION_ERROR", message="Сравнение: от 2 до 5 offer_ids",
                           status_code=400)
        offers = list(ServiceOffer.objects.filter(pk__in=ids,
                                                  tenant_id=request.user.tenant_id))
        if len(offers) != len(ids):
            raise ApiError(code="NOT_FOUND", message="Часть предложений не найдена",
                           status_code=404)
    ...
    def post(self, request, order_id):
        ...
        if offer_id:
            ...
            service = OrderService.objects.create(
                tenant_id=order.tenant_id, order=order, kind=offer.kind,
                title=_offer_title(offer), supplier=offer.supplier,
                source=OrderService.Source.MANUAL if offer.is_manual else OrderService.Source.API,
                currency=offer.price_currency, supplier_cost=offer.price_amount,
            ...
            service = serializer.save(tenant_id=order.tenant_id, order=order,
                                      source=OrderService.Source.MANUAL,
                                      created_by=request.user)
        emit_event("service.updated", service, payload={"action": "created",
...
def _get_service(request, service_id) -> OrderService:
    service = OrderService.objects.filter(pk=service_id,
                                          tenant_id=request.user.tenant_id).first()
    if service is None:
        raise ApiError(code="NOT_FOUND", message="Услуга не найдена", status_code=404)
    get_order_or_404(request.user, service.order_id)  # object scope
    return service
    ...
    def post(self, request, service_id):
        ...
        with transaction.atomic():
            service = OrderService.objects.select_for_update().get(
                pk=_get_service(request, service_id).pk
            )
            ...
            action_map = {"booked": "book", "issued": "issue",
                          "refund_in_progress": "refund", "cancelled": "cancel"}
            if action := action_map.get(target):
                if not has_permission(request.user, f"services.{action}"):
                    raise ApiError(code="PERMISSION_DENIED",
                                   message=f"Нет права services.{action}", status_code=403)
                if not has_service_action(request.user, service.kind, action):
                    raise ApiError(code="SERVICE_KIND_FORBIDDEN",
                                   message=f"Нет доступа к {action} для {service.kind}",
                                   status_code=403)
            # issued требует подтверждённого provider result (ТЗ §8.1);
            # прямой transition в issued разрешён только для manual услуг.
            if target == "issued" and service.source == OrderService.Source.API \
                    and not service.external_id:
                raise ApiError(code="ISSUE_REQUIRES_PROVIDER_RESULT",
            ...
            emit_event("service.updated", service,
                       payload={"action": "status_changed", "from": old_status, "to": target})
            event = audit("services.status_changed", actor=request.user, resource=service,
                          request=request, reason=str(request.data.get("reason", "")),
                          before={"status": old_status}, after={"status": target})
...
class ServiceExtrasView(APIView):
    ...
    def post(self, request, service_id):
        ...
        serializer.is_valid(raise_exception=True)
        extra = serializer.save(tenant_id=service.tenant_id, service=service,
                                created_by=request.user)
...
class ServiceResponsibleView(APIView):
    ...
    def put(self, request, service_id):
        ...
        service = _get_service(request, service_id)
        user = User.objects.filter(pk=request.data.get("responsible"),
                                   tenant_id=request.user.tenant_id,
        ...
        audit("services.responsible_changed", actor=request.user, resource=service,
              request=request,
...
class ManualOfferCreateView(APIView):
    ...
    def post(self, request):
        ...
        offer = ServiceOffer.objects.create(
            tenant_id=request.user.tenant_id, kind=kind, supplier=supplier,
            is_manual=True, itinerary=itinerary, fare=request.data.get("fare"),
            price_amount=price["amount"], price_currency=price.get("currency", "USD"),
            availability="manual", created_by=request.user,
        )
        audit("services.manual_offer_created", actor=request.user, resource=offer,
              request=request)
