            ...
            [
                {
                    ...
                    **(
                        ...
                        if has_permission(request.user, "integrations.manage")
                        else {}
                    ),
                }
                for entry in page
...
def _get_incident(request, incident_id) -> IntegrationIncident:
    incident = IntegrationIncident.objects.filter(pk=incident_id, tenant_id=request.user.tenant_id).first()
    if incident is None:
...
def _timeline(incident, action, user, **details):
    IncidentTimelineEntry.objects.create(incident=incident, action=action, actor=user, details=details)
...
class IncidentAssignView(APIView):
    ...
    def post(self, request, incident_id):
        ...
        assignee = User.objects.filter(
            pk=request.data.get("assignee"), tenant_id=request.user.tenant_id
        ).first()
        if assignee is None:
            raise ApiError(code="VALIDATION_ERROR", message="assignee не найден", status_code=400)
...
class IncidentRetryView(APIView):
    ...
    def post(self, request, incident_id):
        ...
        if incident.job is None:
            raise ApiError(code="NO_JOB", message="Нет связанного задания для повтора", status_code=409)
        job = enqueue(
            incident.job.kind, incident.job.payload, correlation_id=incident.correlation_id, request=request
        )
...
class IncidentSnoozeView(APIView):
    ...
    def post(self, request, incident_id):
        ...
        if not until:
            raise ApiError(code="VALIDATION_ERROR", message="until обязателен", status_code=400)
...
class IncidentSwitchSupplierView(APIView):
    ...
    def post(self, request, incident_id):
        ...
        supplier = Supplier.objects.filter(
            pk=request.data.get("supplier"), tenant_id=request.user.tenant_id
        ).first()
        if supplier is None:
            raise ApiError(code="VALIDATION_ERROR", message="supplier не найден", status_code=400)
        incident.fallback_supplier = supplier
        incident.save(update_fields=["fallback_supplier", "updated_at"])
        _timeline(incident, "supplier_switched", request.user, supplier=str(supplier.pk))
        return Response(IncidentSerializer(incident).data)
...
class IncidentResolveView(APIView):
    ...
    def post(self, request, incident_id):
        ...
        if not resolution_code:
            raise ApiError(
                code="RESOLUTION_CODE_REQUIRED", message="Закрытие требует resolution code", status_code=400
            )
        ...
        incident.save(update_fields=["status", "resolution_code", "resolution_comment", "updated_at"])
        _timeline(incident, "resolved", request.user, resolution_code=resolution_code)
        audit("integrations.incident_resolved", actor=request.user, resource=incident, request=request)
        return Response(IncidentSerializer(incident).data)
...
class IncidentEscalateView(APIView):
    ...
    def post(self, request, incident_id):
        ...
        _timeline(incident, "escalated", request.user, ticket=incident.developer_ticket)
        return Response(IncidentSerializer(incident).data)
