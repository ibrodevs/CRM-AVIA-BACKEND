"""Интеграции: журнал запросов, каталог ошибок, инциденты (ТЗ §13, §21.3)."""
...
class IntegrationLog(models.Model):
    ...
    tenant = models.ForeignKey("tenancy.Organization", null=True, blank=True,
                               on_delete=models.CASCADE, related_name="+")
    correlation_id = models.CharField(max_length=64, blank=True)
    supplier = models.ForeignKey("suppliers.Supplier", null=True, blank=True,
                                 on_delete=models.SET_NULL, related_name="integration_logs")
    provider_adapter = models.CharField(max_length=100)
    operation = models.CharField(max_length=100)  # search/book/issue/refund/...
    ...
    result = models.CharField(max_length=16, blank=True)  # success/error/timeout/unknown
    error_code = models.CharField(max_length=100, blank=True)
    raw_error = models.TextField(blank=True)  # только для admin/debug permission
...
class IntegrationErrorCode(models.Model):
    ...
    category = models.CharField(max_length=32)  # auth/timeout/availability/passenger/issue_unknown/payment/sync/internal
...
class IntegrationIncident(models.Model):
    ...
    severity = models.CharField(max_length=10, default="medium")  # critical/high/medium/low
    ...
    fallback_supplier = models.ForeignKey("suppliers.Supplier", null=True, blank=True,
                                          on_delete=models.SET_NULL, related_name="+")
...
class IncidentTimelineEntry(models.Model):
    ...
    id = models.BigAutoField(primary_key=True)
    incident = models.ForeignKey(IntegrationIncident, on_delete=models.CASCADE,
                                 related_name="timeline")
    action = models.CharField(max_length=32)  # created/assigned/retried/snoozed/resolved/...
