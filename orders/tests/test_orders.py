...
def person(tenant, admin_user):
    return Person.objects.create(tenant=tenant, surname="Иванов", given_name="Пётр",
                                 phone="+996700000001", created_by=admin_user)
...
class TestOrderCreate:
    ...
    def test_history_written(self, admin_client, order_payload):
        order_id = admin_client.post("/api/v1/orders/", order_payload,
                                     format="json").json()["id"]
...
class TestOrderStatusMachine:
    ...
    def _transition(self, client, order, target, **extra):
        return client.post(f"/api/v1/orders/{order['id']}/transition/",
    ...
    def test_valid_chain(self, admin_client, order_payload):
        ...
        for target in ["in_progress", "awaiting_confirmation", "awaiting_payment", "paid",
                       "completed"]:
    ...
    def test_version_conflict(self, admin_client, order_payload):
        ...
        response = admin_client.post(
            ...
            {"target_status": "in_progress", "version": 99}, format="json",
        )
        assert response.status_code == 409
    ...
    def test_manual_paid_requires_finance_permission(self, operator_user, tenant, person):
        ...
        order = self._create(client, {
            "client_person": str(person.id), "request_type": "individual",
        })
    ...
    def test_terminal_rollback_admin_only(self, admin_client, operator_user, order_payload,
                                          person):
        order = self._create(admin_client, order_payload)
        for target in ["in_progress", "cancelled"]:
            order = self._transition(admin_client, order, target,
                                     reason="test").json()
        # оператор не может откатить
        ...
        # админ может, но только с причиной
    ...
    def test_status_not_patchable(self, admin_client, order_payload):
        order = self._create(admin_client, order_payload)
        response = admin_client.patch(f"/api/v1/orders/{order['id']}/",
                                      {"status": "paid", "version": order["version"]},
                                      format="json")
        assert response.status_code == 400
...
class TestOrderScope:
    def test_operator_sees_only_assigned(self, admin_client, operator_user, tenant,
                                         order_payload):
        # заказ создан админом и назначен админу
    ...
    def test_reassign_makes_visible(self, admin_client, operator_user, order_payload):
        ...
        response = admin_client.post(
            ...
            {"operator": str(operator_user.id), "version": order["version"],
             "reason": "передача"},
            format="json",
        )
        assert response.status_code == 200
...
class TestOrderMisc:
    def test_overview(self, admin_client, order_payload):
        ...
        assert set(body) == {"order", "allowed_actions", "services", "finance_summary",
                             "deadlines", "warnings"}
