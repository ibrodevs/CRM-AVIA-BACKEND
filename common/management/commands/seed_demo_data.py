"""Демонстрационные данные backend (ТЗ §26).

Повторяемая и идемпотентная команда. Запрещено запускать в production:
команда проверяет DEBUG и требует --force при DEBUG=False.
"""
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--force", action="store_true",
                            help="Разрешить запуск при DEBUG=False")
    ...
    def handle(self, *args, **options):
        ...
        from crm.models import (
            Agreement, ClientProfile, Company, Contract, FeeRule, Person, PersonDocument,
            SettlementProfile,
        )
        ...
        tenant = _get_or_create(Organization, slug="travelhub",
                                defaults={"name": "Travel Hub"}, report_key="tenant")
        ...
        with tenant_context(tenant.id):
            ...
            for email, role_code, first, last in [
                ...
                if user is None:
                    user = User.objects.create_user(
                        email=email, password="Demo-Pass-2026!", tenant=tenant,
                        status=User.Status.ACTIVE, first_name=first, last_name=last,
                    )
            ...
            # --- клиенты (даты нормализованы, статусы машинные — ТЗ §26) ---
            ivanov = _get_or_create(
                Person, tenant=tenant, surname="Иванов", given_name="Пётр",
                defaults={
                    "middle_name": "Сергеевич", "latin_surname": "IVANOV",
                    "latin_given_name": "PETR", "birth_date": "1988-04-12",
                    "gender": "male", "citizenship": "KG",
                    "phone": "+996700111222", "email": "p.ivanov@example.com",
                    "city": "Бишкек", "created_by": users["admin"],
                }, report_key="person:ivanov")
            if not ivanov.documents.exists():
                PersonDocument.objects.create(
                    tenant=tenant, person=ivanov, type="foreign_passport",
                    number="AC3456789", issuing_country="KG",
                    issued_at="2020-01-15", expires_at="2030-01-15",
                    created_by=users["admin"],
                )
                REPORT["created"].append("document:ivanov")
            _get_or_create(ClientProfile, tenant=tenant, person=ivanov,
                           defaults={"status": "active", "source": "phone",
                                     "assigned_manager": users["manager"],
                                     "created_by": users["admin"]},
                           report_key="client:ivanov")

            company = _get_or_create(
                Company, tenant=tenant, tax_id="01508201910114",
                defaults={
                    "legal_name": "ОсОО «Азия Трэвел Групп»", "short_name": "АТГ",
                    "type": "llc", "legal_address": "Бишкек, пр. Чуй 155",
                    "phone": "+996312900100", "email": "office@atg.example.com",
                    "director": "Касымов Т.Б.",
                    "assigned_manager": users["manager"],
                    "created_by": users["admin"],
                }, report_key="company:atg")
            _get_or_create(SettlementProfile, company=company,
                           defaults={"tenant": tenant, "mode": "deposit",
                                     "currency": "USD",
                                     "deposit_balance": Decimal("5000.00"),
                                     "credit_limit": Decimal("10000.00")},
                           report_key="settlement:atg")
            contract = _get_or_create(Contract, tenant=tenant, company=company,
                                      number="ДОГ-2026/014",
            ...
            agreement = _get_or_create(Agreement, contract=contract, agreement_version=1,
                                       defaults={"tenant": tenant, "number": "СОГЛ-1",
                                                 "status": "active",
                                                 "effective_from": "2026-01-10",
                                                 "created_by": users["admin"]},
                                       report_key="agreement:atg")
            if not agreement.fee_rules.exists():
                FeeRule.objects.create(tenant=tenant, agreement=agreement,
                                       service_kind="avia", fee_kind="service",
                                       calculation="fixed", value=Decimal("15"),
                                       currency="USD",
                                       description="Сервисный сбор за сегмент",
                                       created_by=users["admin"])
                REPORT["created"].append("fee_rule:atg")
            _get_or_create(TravelPolicy, tenant=tenant, company=company,
                           name="Базовая политика",
                           defaults={"allowed_avia_cabins": ["economy"],
                                     "price_limits": {"avia": {"amount": "800",
                                                               "currency": "USD"}},
                                     "created_by": users["admin"]},
            ...
            supplier = _get_or_create(
                Supplier, tenant=tenant, name="Demo GDS",
                defaults={"status": "active", "organization_type": "gds",
                          "service_kinds": ["avia", "rail", "hotel"],
                          "currencies": ["USD", "EUR"],
                          "created_by": users["admin"]},
                report_key="supplier:demo-gds")
            _get_or_create(SupplierMarkupRule, tenant=tenant, supplier=supplier,
                           service_kind="avia", amount_type="percent",
                           defaults={"amount_value": Decimal("5"), "priority": 10,
                                     "created_by": users["admin"]},
                           report_key="markup:demo-gds")
            _get_or_create(
                Supplier, tenant=tenant, name="Гостиницы Иссык-Куля",
                defaults={"status": "active", "organization_type": "hotel_chain",
                          "service_kinds": ["hotel"], "is_global": False,
                          "communication_methods": ["email", "phone"],
                          "created_by": users["admin"]},
                report_key="supplier:hotels")

            # --- заказы (номера только через backend-счётчик — ТЗ §26) ---
            from orders.models import Order

            if not Order.objects.filter(tenant=tenant).exists():
                order = create_order(
                    tenant_id=tenant.id, user=users["operator"],
                    data={
                ...
                corporate = create_order(
                    tenant_id=tenant.id, user=users["manager"],
                    data={
                ...
                FinancialObligation.objects.create(
                    tenant=tenant, order=corporate, direction="client_receivable",
                    currency="USD", original_amount=Decimal("1720.00"),
