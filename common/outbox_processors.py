"""Реестр обработчиков outbox-событий (side effects после commit).

Приложения регистрируют обработчики по типу события (поддерживается
префикс с '*', например "order.*"). run_jobs вызывает их для каждого
необработанного события и помечает событие processed.
"""
import fnmatch
