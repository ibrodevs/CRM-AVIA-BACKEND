"""GET /meta/ — справочники, permissions и feature flags для frontend (ТЗ §20, §21.1)."""
    ...
    def get(self, request):
        ...
        return Response({
            "user": {
                ...
                "roles": [ur.role.code for ur in
                          request.user.user_roles.select_related("role")],
            },
            "enums": {
                "order_statuses": [
                    {"code": code, "display": label}
                    for code, label in Order.Status.choices
                ],
                "order_stages": [
                    {"code": code, "display": label}
                    for code, label in Order.Stage.choices
                ],
                "order_transitions": {k: sorted(v) for k, v in ORDER_TRANSITIONS.items()},
                "service_kinds": SERVICE_KINDS,
                "service_statuses": [
                    {"code": code, "display": label}
                    for code, label in OrderService.Status.choices
                ],
                "priorities": [
                    {"code": code, "display": label}
                    for code, label in Order.Priority.choices
                ],
                "request_types": [
                    {"code": code, "display": label}
                    for code, label in Order.RequestType.choices
                ],
            },
            "settings": {
