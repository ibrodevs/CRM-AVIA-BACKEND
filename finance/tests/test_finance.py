...
def order(admin_client, tenant, admin_user):
    person = Person.objects.create(tenant=tenant, surname="Плательщик", given_name="Тест",
                                   created_by=admin_user)
    return admin_client.post("/api/v1/orders/", {
...
def obligation(admin_client, order, accountant_user):
    ...
    return accountant.post("/api/v1/finance/obligations/", {
        "order": order["id"], "direction": "client_receivable",
        "currency": "USD", "original_amount": "500.00",
    }, format="json").json()


def create_payment(client, order, amount="500.00", key="pay-1") -> dict:
    response = client.post("/api/v1/finance/payments/", {
        "direction": "incoming", "order": order["id"],
        "amount": amount, "currency": "USD", "method": "bank_transfer",
...
class TestPayments:
    def test_confirm_with_allocation_marks_order_paid(self, admin_client, accountant_user,
                                                      order, obligation):
        accountant = auth_client(accountant_user)
        # заказ в awaiting_payment
        current = order
        ...
        response = accountant.post(
            ...
            format="json", HTTP_IDEMPOTENCY_KEY="confirm-1")
    ...
    def test_double_confirm_idempotent(self, accountant_user, order):
        ...
        first = accountant.post(f"/api/v1/finance/payments/{payment['id']}/confirm/",
                                {"version": payment["version"]}, format="json",
                                HTTP_IDEMPOTENCY_KEY="confirm-2")
        second = accountant.post(f"/api/v1/finance/payments/{payment['id']}/confirm/",
                                 {"version": payment["version"]}, format="json",
                                 HTTP_IDEMPOTENCY_KEY="confirm-2")
        assert first.status_code == second.status_code == 200
        assert LedgerEntry.objects.count() == 2  # проводки не задвоились
    ...
    def test_allocation_cannot_exceed_payment(self, accountant_user, order, obligation):
        ...
        accountant.post(f"/api/v1/finance/payments/{payment['id']}/confirm/",
                        {"version": payment["version"]}, format="json",
    ...
    def test_manager_cannot_create_payment(self, manager_user, order):
        ...
        response = manager.post("/api/v1/finance/payments/", {
            "direction": "incoming", "amount": "10.00", "currency": "USD",
        }, format="json", HTTP_IDEMPOTENCY_KEY="pay-m")
...
class TestRefunds:
    def test_refund_formula(self, accountant_user, order):
        ...
        response = accountant.post("/api/v1/finance/refunds/", {
            "payment": payment["id"], "currency": "USD",
            "original_paid": "500.00", "supplier_penalty": "125.00",
            "agency_service_fee": "25.00",
        }, format="json", HTTP_IDEMPOTENCY_KEY="refund-1")
        assert response.status_code == 201, response.content
        body = response.json()
    ...
    def test_refund_cannot_exceed_paid(self, accountant_user, order):
        ...
        response = accountant.post("/api/v1/finance/refunds/", {
            "payment": payment["id"], "currency": "USD", "original_paid": "500.00",
        }, format="json", HTTP_IDEMPOTENCY_KEY="refund-2")
        assert response.status_code == 422
...
class TestAfterSales:
    def _case(self, client, order, service_id=None) -> dict:
        response = client.post("/api/v1/after-sales/", {
            "order": order["id"], "type": "refund", "currency": "USD",
    ...
    def test_full_refund_case_flow(self, admin_client, order):
        ...
        cid = case["id"]
        # review
        admin_client.post(f"/api/v1/after-sales/{cid}/transition/",
                          {"target_status": "review"}, format="json")
        # quote v1
        quote = admin_client.post(f"/api/v1/after-sales/{cid}/quote/", {
            "currency": "USD", "original_paid": "400.00", "supplier_penalty": "100.00",
        }, format="json").json()
        assert quote["refund_total"] == "300.00"
        # send for approval + client approve
        admin_client.post(f"/api/v1/after-sales/{cid}/send-for-approval/", {},
                          format="json")
        response = admin_client.post(f"/api/v1/after-sales/{cid}/client-approve/",
                                     {"quote_version": 1}, format="json")
        assert response.status_code == 200
        # submit + execute
        admin_client.post(f"/api/v1/after-sales/{cid}/submit-to-supplier/", {},
                          format="json", HTTP_IDEMPOTENCY_KEY=f"submit-{cid}")
        response = admin_client.post(f"/api/v1/after-sales/{cid}/execute/", {},
                                     format="json", HTTP_IDEMPOTENCY_KEY=f"exec-{cid}")
        assert response.status_code == 200, response.content
        assert response.json()["status"] == "completed"
        # история полная
        history = admin_client.get(f"/api/v1/after-sales/{cid}/history/").json()
        actions = [h["action"] for h in history]
        assert "created" in actions and "quote_created" in actions \
            and "client_approved" in actions
    ...
    def test_new_quote_invalidates_approval(self, admin_client, order):
        ...
        admin_client.post(f"/api/v1/after-sales/{cid}/transition/",
                          {"target_status": "review"}, format="json")
        admin_client.post(f"/api/v1/after-sales/{cid}/quote/",
                          {"currency": "USD", "original_paid": "400.00"},
                          format="json")
        admin_client.post(f"/api/v1/after-sales/{cid}/send-for-approval/", {},
                          format="json")
        admin_client.post(f"/api/v1/after-sales/{cid}/client-approve/",
                          {"quote_version": 1}, format="json")
        # новая версия quote аннулирует согласие
        admin_client.post(f"/api/v1/after-sales/{cid}/quote/",
                          {"currency": "USD", "original_paid": "400.00",
                           "supplier_penalty": "200.00"}, format="json")
        response = admin_client.post(f"/api/v1/after-sales/{cid}/submit-to-supplier/", {},
                                     format="json", HTTP_IDEMPOTENCY_KEY=f"submit2-{cid}")
        assert response.status_code == 422
        assert response.json()["error"]["code"] == "CLIENT_APPROVAL_REQUIRED"

    def test_execute_requires_processing(self, admin_client, order):
        case = self._case(admin_client, order)
        response = admin_client.post(f"/api/v1/after-sales/{case['id']}/execute/", {},
                                     format="json",
