...
class FinancialAccount(TenantModel):
    ...
    company = models.ForeignKey(
        "crm.Company", null=True, blank=True, on_delete=models.PROTECT, related_name="finance_accounts"
    )
    supplier = models.ForeignKey(
        "suppliers.Supplier", null=True, blank=True, on_delete=models.PROTECT, related_name="finance_accounts"
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=["tenant", "code", "currency"], name="uniq_account_code"),
        ]
...
class LedgerTransaction(models.Model):
    ...
    order = models.ForeignKey(
        "orders.Order", null=True, blank=True, on_delete=models.PROTECT, related_name="ledger_transactions"
    )
    payment = models.ForeignKey(
        "finance.Payment", null=True, blank=True, on_delete=models.PROTECT, related_name="ledger_transactions"
    )
...
class LedgerEntry(models.Model):
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(condition=models.Q(amount__gt=0), name="check_entry_positive"),
        ]
...
class FinancialObligation(TenantModel):
    ...
    order = models.ForeignKey(
        "orders.Order", null=True, blank=True, on_delete=models.PROTECT, related_name="obligations"
    )
    service = models.ForeignKey(
        "services.OrderService", null=True, blank=True, on_delete=models.PROTECT, related_name="obligations"
    )
    aftersale_case = models.ForeignKey(
        "aftersales.AfterSaleCase",
        null=True,
        blank=True,
...
class Payment(TenantModel):
    ...
    direction = models.CharField(max_length=8, choices=Direction.choices, default=Direction.INCOMING)
    order = models.ForeignKey(
        "orders.Order", null=True, blank=True, on_delete=models.PROTECT, related_name="payments"
    )
    payer_person = models.ForeignKey(
        "crm.Person", null=True, blank=True, on_delete=models.PROTECT, related_name="payments"
    )
    payer_company = models.ForeignKey(
        "crm.Company", null=True, blank=True, on_delete=models.PROTECT, related_name="payments"
    )
    supplier = models.ForeignKey(
        "suppliers.Supplier", null=True, blank=True, on_delete=models.PROTECT, related_name="payments"
    )
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(condition=models.Q(amount__gt=0), name="check_payment_positive"),
...
class PaymentAllocation(TenantModel):
    ...
    obligation = models.ForeignKey(FinancialObligation, on_delete=models.PROTECT, related_name="allocations")
    amount = models.DecimalField(max_digits=14, decimal_places=2)

    class Meta:
        db_table = "finance_payment_allocation"
        constraints = [
            models.CheckConstraint(condition=models.Q(amount__gt=0), name="check_allocation_positive"),
        ]
...
class Refund(TenantModel):
    ...
    obligation = models.ForeignKey(
        FinancialObligation, null=True, blank=True, on_delete=models.PROTECT, related_name="refunds"
    )
    aftersale_case = models.ForeignKey(
        "aftersales.AfterSaleCase", null=True, blank=True, on_delete=models.PROTECT, related_name="refunds"
    )
    currency = models.CharField(max_length=3)
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                condition=models.Q(refund_amount__gte=0), name="check_refund_non_negative"
            ),
        ]
...
class ExchangeRate(TenantModel):
    ...
    class Meta:
        ...
        indexes = [models.Index(fields=["tenant", "from_currency", "to_currency", "-as_of"])]
...
class Invoice(TenantModel):
    ...
    currency = models.CharField(max_length=3)
    due_date = models.DateField(null=True, blank=True)
    status = models.CharField(max_length=12, default="issued")
...
class ReconciliationImport(TenantModel):
    ...
    status = models.CharField(max_length=12, default="parsed")
...
class ReconciliationRow(models.Model):
    id = models.BigAutoField(primary_key=True)
    import_job = models.ForeignKey(ReconciliationImport, on_delete=models.CASCADE, related_name="rows")
    ...
    matched_payment = models.ForeignKey(
        Payment, null=True, blank=True, on_delete=models.SET_NULL, related_name="+"
    )
    match_type = models.CharField(max_length=10, blank=True)
    status = models.CharField(max_length=12, default="unmatched")
...
class SalaryAccrual(TenantModel):
    user = models.ForeignKey("accounts.User", on_delete=models.PROTECT, related_name="salary_accruals")
