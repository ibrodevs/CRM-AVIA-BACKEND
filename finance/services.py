"""Финансовые операции: подтверждение платежа, аллокации, возвраты, депозит (ТЗ §14)."""
...
from finance.models import (
    FinancialAccount, FinancialObligation, LedgerEntry, LedgerTransaction, Payment,
...
def _system_account(tenant_id, kind: str, currency: str) -> FinancialAccount:
    account, _ = FinancialAccount.objects.get_or_create(
        tenant_id=tenant_id, code=f"SYS-{kind.upper()}", currency=currency,
        defaults={"name": f"Системный счёт {kind}", "kind": kind},
    )
    return account


def post_ledger(tenant_id, *, kind: str, description: str, entries: list[dict],
                order=None, payment=None, user=None) -> LedgerTransaction:
    ...
    for entry in entries:
        ...
        by_currency[entry["currency"]] = by_currency.get(entry["currency"], Decimal(0)) \
            + sign * entry["amount"]
    ...
    ledger_transaction = LedgerTransaction.objects.create(
        tenant_id=tenant_id, occurred_at=timezone.now(), description=description,
        kind=kind, order=order, payment=payment, posted_by=user,
    )
    LedgerEntry.objects.bulk_create([
        LedgerEntry(transaction=ledger_transaction, account=entry["account"],
...
@transaction.atomic
def confirm_payment(*, payment_id, user, allocations: list[dict] | None = None,
                    expected_version=None, request=None) -> Payment:
    ...
    if payment.status == Payment.Status.CONFIRMED:
        return payment  # идемпотентность на уровне статуса
    if payment.status not in (Payment.Status.DRAFT, Payment.Status.PENDING):
        raise ApiError(code="PAYMENT_NOT_CONFIRMABLE",
                       message=f"Платёж в статусе {payment.status}", status_code=409)

    payment.status = Payment.Status.CONFIRMED
    payment.confirmed_at = timezone.now()
    payment.confirmed_by = user
    payment.version += 1
    payment.save(update_fields=["status", "confirmed_at", "confirmed_by", "version",
                                "updated_at"])

    # проводки: касса/банк <- дебиторка клиента
    ...
    if payment.direction == Payment.Direction.INCOMING:
        entries = [
            {"account": cash, "direction": "debit", "amount": payment.amount,
             "currency": payment.currency},
            {"account": receivable, "direction": "credit", "amount": payment.amount,
             "currency": payment.currency},
        ]
    else:
        payable = _system_account(payment.tenant_id, "supplier_payable", payment.currency)
        entries = [
            {"account": payable, "direction": "debit", "amount": payment.amount,
             "currency": payment.currency},
            {"account": cash, "direction": "credit", "amount": payment.amount,
             "currency": payment.currency},
        ]
    post_ledger(payment.tenant_id, kind="payment",
                description=f"Платёж {payment.pk}", entries=entries,
                order=payment.order, payment=payment, user=user)

    for allocation in (allocations or []):
        allocate_payment(payment=payment, obligation_id=allocation["obligation"],
                         amount=Decimal(str(allocation["amount"])), user=user)

    emit_event("order.updated", payment.order or payment,
               payload={"action": "payment_confirmed", "payment": str(payment.pk)})
    audit("finance.payment_confirmed", actor=user, resource=payment, request=request,
...
def allocate_payment(*, payment: Payment, obligation_id, amount: Decimal, user) -> PaymentAllocation:
    if payment.status != Payment.Status.CONFIRMED:
        raise ApiError(code="PAYMENT_NOT_CONFIRMED",
    ...
    if obligation.currency != payment.currency:
        raise BusinessRejectionError(code="CURRENCY_MISMATCH",
                                     message="Валюта платежа и обязательства различаются")
    ...
    allocation = PaymentAllocation.objects.create(
        tenant_id=payment.tenant_id, payment=payment, obligation=obligation,
        amount=amount, created_by=user,
    )
...
def _maybe_mark_order_paid(order, user) -> None:
    ...
    outstanding = FinancialObligation.objects.filter(
        order=order, direction=FinancialObligation.Direction.CLIENT_RECEIVABLE,
        status__in=["open", "partial"],
    ).exists()
    if not outstanding:
        from orders.services import transition_order

        transition_order(order_id=order.pk, user=user, target_status=Order.Status.PAID,
                         reason="Оплата полностью распределена",
                         expected_version=Order.objects.get(pk=order.pk).version)


def build_refund(*, tenant_id, currency: str, original_paid: Decimal,
                 supplier_penalty: Decimal = Decimal(0),
                 agency_service_fee: Decimal = Decimal(0),
                 other_withholdings: Decimal = Decimal(0),
                 payment: Payment | None = None, obligation=None, aftersale_case=None,
                 user=None) -> Refund:
    ...
    if payment is not None:
        already = payment.refunds.filter(status__in=["approved", "executed"]).aggregate(
            total=models.Sum("refund_amount"))["total"] or Decimal(0)
    ...
    return Refund.objects.create(
        tenant_id=tenant_id, payment=payment, obligation=obligation,
        aftersale_case=aftersale_case, currency=currency,
        original_paid=original_paid, supplier_penalty=supplier_penalty,
        agency_service_fee=agency_service_fee, other_withholdings=other_withholdings,
...
def execute_refund(*, refund_id, user, request=None) -> Refund:
    ...
    if refund.status not in (Refund.Status.DRAFT, Refund.Status.APPROVED):
        raise ApiError(code="REFUND_NOT_EXECUTABLE",
                       message=f"Возврат в статусе {refund.status}", status_code=409)
    refund.status = Refund.Status.EXECUTED
    refund.executed_at = timezone.now()
    refund.executed_by = user
    refund.version += 1
    refund.save(update_fields=["status", "executed_at", "executed_by", "version",
                               "updated_at"])
    ...
    if refund.refund_amount > 0:
        post_ledger(refund.tenant_id, kind="refund",
                    description=f"Возврат {refund.pk}",
                    entries=[
                        {"account": receivable, "direction": "debit",
                         "amount": refund.refund_amount, "currency": refund.currency},
                        {"account": cash, "direction": "credit",
                         "amount": refund.refund_amount, "currency": refund.currency},
                    ],
                    payment=refund.payment, user=user)
    if refund.obligation_id:
        obligation = FinancialObligation.objects.select_for_update().get(
            pk=refund.obligation_id)
        obligation.refunded_amount += refund.refund_amount
        obligation.save(update_fields=["refunded_amount", "updated_at"])
    audit("finance.refund_executed", actor=user, resource=refund, request=request,
          after={"amount": str(refund.refund_amount)})
    emit_event("order.updated",
...
@transaction.atomic
def reserve_deposit(*, company, amount: Decimal, user, allow_override: bool = False,
                    reason: str = "") -> None:
    ...
    if amount > available and allow_override:
        if not reason:
            raise ApiError(code="REASON_REQUIRED", message="Override требует причины",
                           status_code=400)
        audit("finance.deposit_override", actor=user, resource=company, reason=reason,
              after={"requested": str(amount), "available": str(available)})
        # override поверх constraint: увеличиваем баланс виртуально нельзя —
        # фиксируем перерасход отдельным полем невозможно; допускаем reserve=balance
