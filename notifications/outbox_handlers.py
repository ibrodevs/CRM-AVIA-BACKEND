"""Создание уведомлений из domain events (outbox-процессор) и проверка дедлайнов."""
...
# Человекочитаемые заголовки для базовых типов событий.
_TITLES = {
...
def create_notifications(event: OutboxEvent) -> None:
    ...
    if event.tenant_id is None or event.event_type.startswith("notification."):
        return
    rules = NotificationRule.objects.filter(tenant_id=event.tenant_id, is_active=True,
    ...
    for rule in rules:
        ...
        for user in _resolve_recipients(rule, event):
            notification = Notification.objects.create(
                tenant_id=event.tenant_id, user=user, priority=rule.priority,
                source=event.event_type.split(".")[0], event_type=event.event_type,
            ...
            emit_event("notification.created", notification,
                       ...
                       audience_user=user, tenant_id=event.tenant_id)
...
def _resolve_recipients(rule: NotificationRule, event: OutboxEvent):
    ...
    if roles := recipients.get("roles"):
        users.update(User.objects.filter(user_roles__role__code__in=roles,
                                         tenant_id=event.tenant_id,
                                         status=User.Status.ACTIVE))
...
def check_deadlines() -> str:
    ...
    for hours, threshold in ((24, "24h"), (2, "2h")):
        ...
        for service in services:
            ...
            _, was_created = DeadlineThreshold.objects.get_or_create(
                rule_key="ticketing_deadline", resource_type="OrderService",
                resource_id=str(service.pk), threshold=threshold, recipient=recipient,
            )
            if was_created:
                Notification.objects.create(
                    tenant_id=service.tenant_id, user=recipient,
                    priority="critical" if threshold == "2h" else "high",
                    source="services", event_type="service.deadline",
                    title=f"Дедлайн выписки через {threshold}",
                    body=f"{service.title}: выписка до {service.ticketing_deadline}",
                    resource_type="OrderService", resource_id=str(service.pk),
                )
                created += 1
    # просроченные
    overdue = OrderService.objects.filter(
        ticketing_deadline__lt=now, status__in=["booked", "confirmed"],
    ).select_related("order")
    for service in overdue:
        ...
        _, was_created = DeadlineThreshold.objects.get_or_create(
            rule_key="ticketing_deadline", resource_type="OrderService",
            resource_id=str(service.pk), threshold="overdue", recipient=recipient,
        )
        if was_created:
            Notification.objects.create(
                tenant_id=service.tenant_id, user=recipient, priority="critical",
                source="services", event_type="service.deadline_overdue",
