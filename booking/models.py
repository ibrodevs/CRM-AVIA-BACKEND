"""Оркестрация бронирования и выписки (ТЗ §10)."""
...
class BookingWorkflow(TenantModel):
    class Status(models.TextChoices):
        ...
        RUNNING = "running"
        COMPLETED = "completed"
        PARTIAL = "partial"          # часть услуг забронирована, часть упала
        FAILED = "failed"
        CANCELLED = "cancelled"

    order = models.ForeignKey("orders.Order", on_delete=models.PROTECT,
                              related_name="booking_workflows")
    status = models.CharField(max_length=14, choices=Status.choices, default=Status.DRAFT)
    plan_snapshot = models.JSONField(null=True, blank=True)   # зафиксированный план saga
...
class BookingWorkflowItem(TenantModel):
    ...
    class Status(models.TextChoices):
        ...
        ISSUED = "issued"
        FAILED = "failed"
        UNKNOWN = "unknown"          # ambiguous timeout — повтор только после inquiry
        COMPENSATED = "compensated"  # аннулировано compensating cancellation
        SKIPPED = "skipped"

    workflow = models.ForeignKey(BookingWorkflow, on_delete=models.CASCADE,
                                 related_name="items")
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=["workflow", "service"],
                                    name="uniq_workflow_service"),
        ]
        ordering = ["sequence"]
