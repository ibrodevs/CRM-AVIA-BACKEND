...
class TestPreflight:
    def test_preflight_ok(self, admin_client, ready_order):
        ...
        result = admin_client.post(f"/api/v1/booking-workflows/{workflow['id']}/preflight/").json()
        assert result["ok"] is True, result
        assert result["blocking_errors"] == []

    def test_preflight_blocks_missing_data(self, admin_client, tenant, admin_user):
        person = Person.objects.create(
            tenant=tenant, surname="Без", given_name="Латиницы", created_by=admin_user
        )
        order = admin_client.post(
            "/api/v1/orders/",
            {
...
class TestBookingSaga:
    def _booked_workflow(self, client, ready_order) -> dict:
        ...
        response = client.post(
            ...
            {},
            format="json",
            HTTP_IDEMPOTENCY_KEY=f"start-{workflow['id']}",
        )
        assert response.status_code == 202, response.content
        run_jobs_once()
        return client.get(f"/api/v1/booking-workflows/{workflow['id']}/status/").json()
    ...
    def test_booking_success(self, admin_client, ready_order):
        ...
        body = admin_client.get(f"/api/v1/orders/{workflow['order']}/services/").json()["results"][0]
    ...
    def test_issue_success(self, admin_client, ready_order):
        ...
        response = admin_client.post(
            ...
            {},
            format="json",
            HTTP_IDEMPOTENCY_KEY=f"issue-{workflow['id']}",
        )
        assert response.status_code == 202
        run_jobs_once()
        body = admin_client.get(f"/api/v1/booking-workflows/{workflow['id']}/status/").json()
    ...
    def test_issue_unknown_blocks_retry(self, admin_client, ready_order):
        ...
        response = admin_client.post(
            ...
            {"_mock": {"fail": "timeout"}},
            format="json",
            HTTP_IDEMPOTENCY_KEY=f"issue-t-{workflow['id']}",
        )
        assert response.status_code == 202
        run_jobs_once()
        body = admin_client.get(f"/api/v1/booking-workflows/{workflow['id']}/status/").json()
        ...
        response = admin_client.post(
            ...
            {},
            format="json",
            HTTP_IDEMPOTENCY_KEY=f"issue-r-{workflow['id']}",
        )
        assert response.status_code == 409
        ...
        response = admin_client.post(
            f"/api/v1/booking-workflows/{workflow['id']}/status-inquiry/", {"item": item_id}, format="json"
        )
        assert response.status_code == 202
        run_jobs_once()
        body = admin_client.get(f"/api/v1/booking-workflows/{workflow['id']}/status/").json()
        assert body["items"][0]["status"] == "booked"
        from avia.models import Ticket
    ...
    def test_compensating_cancellation(self, admin_client, ready_order):
        ...
        response = admin_client.post(
            ...
            format="json",
            HTTP_IDEMPOTENCY_KEY=f"cancel-{workflow['id']}",
        )
        assert response.status_code == 202
        run_jobs_once()
        body = admin_client.get(f"/api/v1/booking-workflows/{workflow['id']}/status/").json()
