...
from aftersales.models import (
    AFTERSALE_TRANSITIONS, AfterSaleCase, AfterSaleHistoryEntry, AfterSaleNumberCounter,
    AfterSaleQuote,
)
...
class QuoteSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ["id", "quote_version", "source", "currency", "original_paid",
                  "supplier_penalty", "agency_service_fee", "other_withholdings",
                  "refund_total", "old_itinerary", "new_itinerary",
                  "exchange_difference", "details", "created_at"]
...
class CaseSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = AfterSaleCase
        fields = ["id", "number", "order", "service", "type", "initiator", "responsible",
                  "supplier", "status", "deadline", "currency", "financial_snapshot",
                  "external_references", "current_quote", "client_approved_at",
                  "client_approved_quote_version", "quotes", "created_at", "version"]
        read_only_fields = ["id", "number", "status", "current_quote",
                            "client_approved_at", "client_approved_quote_version",
                            "created_at", "version"]


def _get_case(request, case_id) -> AfterSaleCase:
    case = AfterSaleCase.objects.filter(pk=case_id,
...
def _history(case, action, user, **details):
    AfterSaleHistoryEntry.objects.create(case=case, action=action, actor=user,
                                         details=details)
...
def _transition(case: AfterSaleCase, target: str, user, reason: str = "") -> None:
    ...
    emit_event("order.updated", case.order,
...
class CaseListCreateView(GenericAPIView):
    ...
    def get(self, request):
        qs = AfterSaleCase.objects.filter(tenant_id=request.user.tenant_id,
    ...
    def post(self, request):
        needed = {"refund": "services.refund", "exchange": "services.exchange",
                  "cancellation": "services.cancel", "certificate": "orders.change"}
        case_type = str(request.data.get("type", ""))
        permission = needed.get(case_type)
        if permission is None:
            raise ApiError(code="VALIDATION_ERROR",
                           message=f"type из {sorted(needed)}", status_code=400)
        if not has_permission(request.user, permission):
            raise ApiError(code="PERMISSION_DENIED", message=f"Нет права {permission}",
                           status_code=403)
        ...
        audit("aftersales.case_created", actor=request.user, resource=case,
              request=request)
...
class CaseQuoteView(APIView):
    ...
    def post(self, request, case_id):
        data = request.data
        with transaction.atomic():
            case = AfterSaleCase.objects.select_for_update().get(
                pk=_get_case(request, case_id).pk
            )
            try:
            ...
            refund_total = max(quantize(original_paid - penalty - fee - other, currency),
                               Decimal(0))
            last = case.quotes.order_by("-quote_version").first()
            quote = AfterSaleQuote.objects.create(
                tenant_id=case.tenant_id, case=case,
                quote_version=(last.quote_version + 1) if last else 1,
                source=str(data.get("source", "manual")),
                currency=currency, original_paid=original_paid,
                supplier_penalty=penalty, agency_service_fee=fee,
                other_withholdings=other, refund_total=refund_total,
            ...
            # изменение суммы/условий аннулирует согласие клиента (ТЗ §16)
            if case.client_approved_at is not None:
                case.client_approved_at = None
                case.client_approved_quote_version = None
                _history(case, "client_approval_invalidated", request.user,
                         new_quote_version=quote.quote_version)
            case.save(update_fields=["current_quote", "currency", "client_approved_at",
                                     "client_approved_quote_version", "updated_at"])
            _history(case, "quote_created", request.user,
                     quote_version=quote.quote_version, refund_total=str(refund_total))
        audit("aftersales.quote_created", actor=request.user, resource=case,
              request=request)
...
class CaseTransitionView(APIView):
    ...
    def post(self, request, case_id):
        with transaction.atomic():
            ...
            _transition(case, str(request.data.get("target_status", "")), request.user,
    ...
    def post(self, request, case_id):
        with transaction.atomic():
            ...
            if case.current_quote_id is None:
                raise ApiError(code="QUOTE_REQUIRED", message="Сначала создайте quote",
                               status_code=409)
    ...
    def post(self, request, case_id):
        with transaction.atomic():
            ...
            if case.status != AfterSaleCase.Status.AWAITING_CLIENT_APPROVAL:
                raise ApiError(code="INVALID_STATUS",
                               message="Кейс не ожидает согласия клиента", status_code=409)
            quote_version = request.data.get("quote_version")
            if case.current_quote is None or \
                    int(quote_version or 0) != case.current_quote.quote_version:
                raise BusinessRejectionError(
                    code="QUOTE_VERSION_MISMATCH",
                    message="Согласие относится не к актуальной версии расчёта",
                    details={"current_version": case.current_quote.quote_version
                             if case.current_quote else None},
                )
            case.client_approved_at = timezone.now()
            case.client_approved_quote_version = case.current_quote.quote_version
            case.save(update_fields=["client_approved_at",
                                     "client_approved_quote_version", "updated_at"])
            _history(case, "client_approved", request.user,
                     quote_version=case.client_approved_quote_version)
        return Response(CaseSerializer(case).data)
    ...
    def post(self, request, case_id):
        ...
        with transaction.atomic():
            ...
            if case.status != AfterSaleCase.Status.PROCESSING:
                raise ApiError(code="INVALID_STATUS",
                               message="Завершать можно только кейс в processing",
                               status_code=409)
            ...
            if case.type == "refund":
                ...
                if quote is not None:
                    refund = finance_service.build_refund(
                        tenant_id=case.tenant_id, currency=quote.currency,
                        ...
                        aftersale_case=case, user=request.user,
                    )
                    finance_service.execute_refund(refund_id=refund.pk,
                    ...
                    if not request.data.get("reason"):
                        raise ApiError(code="REASON_REQUIRED",
                                       ...
                                       status_code=400)
                    _history(case, "manual_exception", request.user,
            ...
            if case.service:
                ...
                target = {"refund": OrderService.Status.REFUNDED,
                          "cancellation": OrderService.Status.CANCELLED}.get(case.type)
                if target:
                    service.status = target
                    service.version += 1
                    service.save(update_fields=["status", "version", "updated_at"])
            case.save(update_fields=["financial_snapshot", "updated_at"])
        audit("aftersales.case_executed", actor=request.user, resource=case,
              request=request)
        return Response(CaseSerializer(case).data)
...
class CaseCancelView(APIView):
    ...
    def post(self, request, case_id):
        with transaction.atomic():
            ...
            _transition(case, AfterSaleCase.Status.CANCELLED, request.user,
                        reason=str(request.data.get("reason", "")))
        return Response(CaseSerializer(case).data)
...
class CaseDocumentsView(APIView):
    ...
    def get(self, request, case_id):
        ...
        from documents.views import DocumentSerializer
    ...
    def get(self, request, case_id):
        ...
        return Response([
            {"id": h.id, "action": h.action,
             ...
             "details": h.details, "created_at": h.created_at}
