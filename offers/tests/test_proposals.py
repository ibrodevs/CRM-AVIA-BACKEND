...
def order(admin_client, tenant, admin_user):
    ...
    person = Person.objects.create(tenant=tenant, surname="Клиент", given_name="КП",
...
PROPOSAL = {
    "type": "travel", "purpose": "Командировка", "currency": "USD",
    "variants": [
        {"name": "Эконом", "items": [
            {"title": "Перелёт FRU-IST", "quantity": 1,
             "price_amount": "450.00", "price_currency": "USD"},
        ]},
        {"name": "Бизнес", "items": [
            {"title": "Перелёт FRU-IST бизнес", "quantity": 1,
             "price_amount": "1200.00", "price_currency": "USD"},
        ]},
    ],
}
...
def create_proposal(client, order) -> dict:
    response = client.post("/api/v1/proposals/", {**PROPOSAL, "order": order["id"]},
                           format="json")
...
class TestProposalLifecycle:
    ...
    def test_full_flow_to_approval(self, admin_client, order):
        proposal = create_proposal(admin_client, order)
        pid = proposal["id"]
        # prepare создаёт immutable версию
        response = admin_client.post(f"/api/v1/proposals/{pid}/prepare/",
                                     {"version": proposal["version"]}, format="json")
        assert response.status_code == 200
        proposal = response.json()
        assert proposal["status"] == "prepared"
        assert proposal["current_version"] == 1
        # send
        response = admin_client.post(f"/api/v1/proposals/{pid}/send/",
                                     {"version": proposal["version"]}, format="json",
        ...
        response = admin_client.post(
            ...
            format="json", HTTP_IDEMPOTENCY_KEY="approve-1",
        )
    ...
    def test_forbidden_transition(self, admin_client, order):
        ...
        response = admin_client.post(
            ...
            format="json", HTTP_IDEMPOTENCY_KEY="a-2",
        )
    ...
    def test_versions_immutable_snapshot(self, admin_client, order):
        proposal = create_proposal(admin_client, order)
        admin_client.post(f"/api/v1/proposals/{proposal['id']}/prepare/",
                          {"version": proposal["version"]}, format="json")
        versions = admin_client.get(
            f"/api/v1/proposals/{proposal['id']}/versions/").json()
    ...
    def test_operator_cannot_approve(self, operator_user, admin_client, order):
        ...
        response = operator.post(
            f"/api/v1/proposals/{proposal['id']}/approve/",
            {"variant": proposal["variants"][0]["id"], "version": proposal["version"]},
            format="json", HTTP_IDEMPOTENCY_KEY="a-3",
        )
...
class TestServiceCards:
    def _card(self, client, order) -> dict:
        response = client.post("/api/v1/service-cards/", {
            "order": order["id"], "kind": "avia",
    ...
    def test_send_and_public_view(self, admin_client, order):
        ...
        response = admin_client.post(f"/api/v1/service-cards/{card['id']}/send/",
                                     {"channels": ["telegram"]}, format="json",
                                     HTTP_IDEMPOTENCY_KEY="card-send-1")
        assert response.status_code == 200
        assert response.json()["status"] == "sent"
        # публичный просмотр без JWT
        public = APIClient()
