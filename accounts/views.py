"""Профиль (/me/) и управление пользователями (/users/) — ТЗ §5.2."""
import hashlib
import secrets
...
from accounts.models import (
    PasswordResetToken, Role, User, UserPreference, UserRole, UserServiceAccess, UserSession,
)
from accounts.permissions import require
from accounts.serializers import (
    MeSerializer, RoleSerializer, UserCreateSerializer, UserPreferenceSerializer,
    UserSerializer, UserServiceAccessSerializer,
)
...
class MeView(APIView):
    ...
    def patch(self, request):
        allowed = {
            "first_name", "last_name", "middle_name", "phone", "work_phone", "internal_phone",
            "telegram", "timezone", "language", "presence", "work_status",
        }
...
class MeAvatarView(APIView):
    def put(self, request):
        ...
        if file.size > 5 * 1024 * 1024:
            raise ApiError(code="FILE_TOO_LARGE", message="Максимальный размер аватара 5 МБ",
                           status_code=400)
        if file.content_type not in ("image/jpeg", "image/png", "image/webp"):
            raise ApiError(code="UNSUPPORTED_FILE_TYPE", message="Допустимы JPEG/PNG/WebP",
                           status_code=400)
...
class UserListCreateView(GenericAPIView):
    ...
    def post(self, request):
        ...
        serializer.is_valid(raise_exception=True)
        user = User(tenant_id=request.user.tenant_id, status=User.Status.INVITED,
...
class UserDetailView(APIView):
    ...
    def patch(self, request, user_id):
        ...
        allowed = {
            "phone", "first_name", "last_name", "middle_name", "position", "department",
            "manager", "work_phone", "internal_phone", "telegram", "hired_at",
            "work_status", "timezone", "language", "sla_response_minutes",
        }
        data = {k: v for k, v in request.data.items() if k in allowed}
        ...
        serializer.save()
        audit("users.updated", request=request, resource=user, before=before,
...
class UserInviteView(APIView):
    ...
    def post(self, request, user_id):
        ...
        if user.status not in (User.Status.INVITED, User.Status.SUSPENDED):
            raise ApiError(code="INVALID_USER_STATUS",
        ...
        # Токен возвращается вызывающему администратору (доставка email — адаптером).
...
class UserSuspendView(APIView):
    ...
    def post(self, request, user_id):
        ...
        if user.pk == request.user.pk:
            raise ApiError(code="CANNOT_SUSPEND_SELF", message="Нельзя заблокировать себя",
                           status_code=409)
        ...
        audit("users.suspended", request=request, resource=user,
...
class UserRolesView(APIView):
    ...
    def put(self, request, user_id):
        ...
        if not isinstance(codes, list):
            raise ApiError(code="VALIDATION_ERROR", message="Ожидается список кодов ролей",
                           fields={"roles": ["Обязательное поле-список"]}, status_code=400)
        roles = list(Role.objects.filter(tenant_id=request.user.tenant_id, code__in=codes))
        missing = set(codes) - {r.code for r in roles}
        if missing:
            raise ApiError(code="UNKNOWN_ROLE", message=f"Неизвестные роли: {sorted(missing)}",
                           status_code=400)
        with transaction.atomic():
        ...
        audit("users.roles_changed", request=request, resource=user,
              before={"roles": before}, after={"roles": codes})
...
class UserServiceAccessView(APIView):
    ...
    def put(self, request, user_id):
        ...
        audit("users.service_access_changed", request=request, resource=user,
...
class UserSlaView(APIView):
    ...
    def put(self, request, user_id):
        ...
        if minutes is not None and (not isinstance(minutes, int) or minutes <= 0):
            raise ApiError(code="VALIDATION_ERROR", message="Некорректный SLA",
        ...
        audit("users.sla_changed", request=request, resource=user,
              after={"sla_response_minutes": minutes})
