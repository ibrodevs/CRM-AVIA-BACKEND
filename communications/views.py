...
def _threads_qs(request):
    qs = ChatThread.objects.filter(tenant_id=request.user.tenant_id, archived_at__isnull=True)
    allowed_types = []
...
class ThreadReadView(APIView):
    ...
    def post(self, request, thread_id):
        ...
        message = (
            thread.messages.filter(pk=message_id).first()
            if message_id
            else thread.messages.order_by("-id").first()
        )
        ...
        emit_event(
            "chat.message.read", thread, payload={"user_id": str(request.user.id)}, audience_user=request.user
        )
...
class ThreadParticipantsView(APIView):
            ...
            [
                {
                    ...
                    "external_identity": p.external_identity,
                    "role": p.role,
    ...
    def post(self, request, thread_id):
        ...
        thread = _get_thread(request, thread_id)
        user = User.objects.filter(pk=request.data.get("user"), tenant_id=request.user.tenant_id).first()
        if user is None:
        ...
        return Response(
            {"id": str(participant.id)}, status=http.HTTP_201_CREATED if created else http.HTTP_200_OK
        )
...
class UnreadCountView(APIView):
    ...
    def get(self, request):
        total = 0
        threads = _threads_qs(request).filter(
        ...
        for thread in threads:
            ...
            qs = thread.messages.filter(deleted_at__isnull=True).exclude(author_user=request.user)
    ...
    def post(self, request, provider):
        ...
        if secret:
            ...
            if not signature_valid:
                raise ApiError(code="INVALID_SIGNATURE", message="Подпись не совпадает", status_code=401)
        external_event_id = str(request.data.get("event_id", ""))
        if not external_event_id:
            raise ApiError(code="VALIDATION_ERROR", message="event_id обязателен", status_code=400)
